...
class MediaSessionManager(
    ...
    private fun getCurrentPosition(): Long =
        playbackManager.player()?.currentStreamPosition() ?: 0L
    ...
    fun updateMetaData(songInfo: SongInfo?) {
        ...
        val duration = when (playbackManager.player()?.duration()) {
            0L, -1L -> songInfo.duration
            else -> playbackManager.player()?.duration() ?: 199899
        }
        Log.d("mediaSessionManager", "duration : $duration")
        val metaDta = MediaMetadataCompat.Builder().apply {
            ...
            putString(MediaMetadataCompat.METADATA_KEY_ALBUM_ART_URI, songInfo.songCover)
            putString(MediaMetadataCompat.METADATA_KEY_MEDIA_URI, songInfo.songCover)
            putLong(MediaMetadataCompat.METADATA_KEY_DURATION, duration)  //加上这个通知栏会有进度条
        }
    ...
    private class MediaSessionCallback(val playbackManager: PlaybackManager) :
