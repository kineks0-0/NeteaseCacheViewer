...
class SystemNotification constructor(
    ...
    init {
        ...
        mStopIntent = config.stopIntent ?: INotification.ACTION_STOP.getPendingIntent()
        mNextIntent = config.nextIntent ?: INotification.ACTION_NEXT.getPendingIntent()
        mPreviousIntent = config.preIntent ?: INotification.ACTION_PREV.getPendingIntent()
        mPlayIntent = config.playIntent ?: INotification.ACTION_PLAY.getPendingIntent()
        mPauseIntent = config.pauseIntent ?: INotification.ACTION_PAUSE.getPendingIntent()
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        when (action) {
            INotification.ACTION_PAUSE -> pauseMusic(player)
            INotification.ACTION_PLAY -> restoreMusic(player)
            INotification.ACTION_NEXT -> player?.skipToNext()
            INotification.ACTION_PREV -> player?.skipToPrevious()
            INotification.ACTION_STOP -> {
                pauseMusic(player)
                player?.stop()
                stopNotification()
            }
    ...
    override fun startNotification(songInfo: SongInfo?, playbackState: String) {
        ...
        if (!mStarted) {
            ...
            if (notification != null) {
                ...
                filter.addAction(INotification.ACTION_STOP)
                context.registerReceiver(this, filter)
                (context as MusicService).startForeground(
                    INotification.NOTIFICATION_ID,
                    notification
                )
                mStarted = true
            }
    ...
    private fun createNotification(): Notification? {
        ...
        notificationBuilder
            ...
            .setColorized(true)
            .setSubText(songInfo?.headData?.get("album"))
    ...
    private fun fetchBitmapFromURLAsync(
        ...
        GlobalScope.launch {
            val loader = ImageLoader(App.context)
    ...
    private fun addActions(notificationBuilder: NotificationCompat.Builder): Int {
        ...
        if (hasPreSong) {
            val icon =
                if (config.skipPreviousDrawableRes != -1) config.skipPreviousDrawableRes else R.drawable.ic_skip_previous_white_24dp
            val title =
                if (config.skipPreviousTitle.isNotEmpty()) config.skipPreviousTitle else context.getString(
        ...
        if (playbackState == PlaybackStage.PLAYING || playbackState == PlaybackStage.BUFFERING) {
            label =
                if (config.labelPlay.isNotEmpty()) config.labelPlay else context.getString(R.string.label_pause)
            icon =
                if (config.pauseDrawableRes != -1) config.pauseDrawableRes else R.drawable.ic_pause_white_24dp
            intent = mPauseIntent
        } else {
            label =
                if (config.labelPause.isNotEmpty()) config.labelPause else context.getString(R.string.label_play)
            icon =
                if (config.playDrawableRes != -1) config.playDrawableRes else R.drawable.ic_play_arrow_white_24dp
            intent = mPlayIntent
        }
        ...
        if (hasNextSong) {
            val actionIcon =
                if (config.skipNextDrawableRes != -1) config.skipNextDrawableRes else R.drawable.ic_skip_next_white_24dp
            val title =
                if (config.skipNextTitle.isNotEmpty()) config.skipNextTitle else context.getString(R.string.label_next)
