...
class MusicService : Service() {
    ...
    private fun initTelephony() {
        if (telephonyManager == null) {
            telephonyManager =
                this.getSystemService(TELEPHONY_SERVICE) as TelephonyManager // 获取电话通讯服务
            val callback: (state: Int) -> Unit = { state ->
                when (state) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                ...
                telephonyManager!!.listen(object : PhoneStateListener() {
                    @Deprecated("Deprecated in Java", ReplaceWith("callback(state)"))
